...
public class TemplateImpl implements Template {
    ...
    TemplateImpl(String template) {
        ...
        if (colList.size() == segList.size()) {
            templateStringWithoutColumnNames = segList.stream()
                    .collect(joining("{}", "", "{}"));
        } else if (colList.size() + 1 == segList.size()) {
            templateStringWithoutColumnNames = segList.stream()
                    .collect(joining("{}", "", ""));
        } else {
     ...
     *
     * @param index The index of the possibly escaped character.
     * @param chars The array of characters to look in.
